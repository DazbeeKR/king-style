...
<!-- 🔹 Promociones exclusivas fijas por día -->
<h2 class="seccion">🔥 Promociones exclusivas</h2>
<div class="container mt-4">
  <div class="productos-row">
  <?php
  // 🔹 Semilla basada en la fecha actual para fijar promociones por día
  $seed = date('Ymd'); // Ej: 20251105
  $conn->query("SET @seed := $seed");

  // 🔹 Obtener 4 productos aleatorios, consistentes durante todo el día
  $promos = $conn->query("SELECT id, nombre, precio, imagen FROM productos ORDER BY RAND(@seed) LIMIT 4");

  // 🔹 Array de posibles descuentos
  $descuentos_disponibles = [5, 10, 15];

  while ($producto = $promos->fetch_assoc()) {
      // 🔹 Descuento “aleatorio” pero consistente: depende del día y del ID del producto
